...
public class ServicePresenter extends BasePresenter<ServiceView, ServiceModel> {
     ...
     * @param requestData 参数
     */
    public void noticeList(Map<String, String> requestData) {
        mRxManage.add(
                mModel.noticeList(requestData, new RxSubscriber<ServiceBean>(mContext) {
                    @Override
                    public void onStart() {
                        super.onStart();
                        mView.showLoading("正在加载,请稍后...");
                    ...
                    @Override
                    protected void _onNext(ServiceBean DataBean) {
                        mView.stopLoading();
