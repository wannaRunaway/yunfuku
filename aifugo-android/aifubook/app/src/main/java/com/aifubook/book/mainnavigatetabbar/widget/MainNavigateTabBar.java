package com.aifubook.book.mainnavigatetabbar.widget;
...
import com.aifubook.book.mainnavigatetabbar.ThemeUtils;
...
public class MainNavigateTabBar extends LinearLayout implements View.OnClickListener {
    ...
    public MainNavigateTabBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mNormalTextColor = (tabTextColor != null ? tabTextColor : context.getResources().getColorStateList(R.color.textcolor_gray));
...
//        }
        if (TextUtils.isEmpty(tabParam.title)) {
            tabParam.title = tabParam.titleStringRes == 0 ? "" : getContext().getString(tabParam.titleStringRes);
        }
    ...
    private void showFragment(ViewHolder holder) {
        androidx.fragment.app.FragmentTransaction transaction = mFragmentActivity.getSupportFragmentManager().beginTransaction();
        if (isFragmentShown(transaction, holder.tag)) {
            return;
        }
        setCurrSelectedTabByTag(holder.tag);

        androidx.fragment.app.Fragment fragment = mFragmentActivity.getSupportFragmentManager().findFragmentByTag(holder.tag);
        if (fragment == null) {
    ...
    private boolean isFragmentShown(FragmentTransaction transaction, String newTag) {
        ...
        androidx.fragment.app.Fragment fragment = mFragmentActivity.getSupportFragmentManager().findFragmentByTag(mCurrentTag);
