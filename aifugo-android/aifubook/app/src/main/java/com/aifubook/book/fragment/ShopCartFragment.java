...
public class ShopCartFragment extends BaseFragment implements OnCallBack, SwipeRefreshLayout.OnRefreshListener, @Nullable OnLoadMoreListener,Addcart {
...
//        barView.setLayoutParams(layoutParams);
        ImageView leftImg = findViewById(R.id.leftImg);
        TextView tv_center = findViewById(R.id.textview);
...
//                bundle.putInt("zeroBuy", 1);
    ...
    private void getRecommendList() {
        String shopId = SharedPreferencesUtil.get(mActivity, "SHOP_ID", "");
...
//        mPresenter.submitCarts(StringUtil.listToString(cartIds));
    ...
    private void onCouponsList(Object result) {
        ...
        if (DataBean != null && DataBean.size() > 0) {
            ...
            showReportDialog.showCouponDialog(getActivity(), DataBean);
...
//        closeLoadingDialog();
    ...
    private void initShopList(List<CartBean.ShopCarsBean> cartList) {
        ...
        for (int i = 0; i < cartList.size(); i++) {
            ...
            tv_get_coupon.setOnClickListener(v -> {
            ...
            cbShopAll.setOnClickListener(v -> {
                int shopId1 = item.getShop().getId();
                for (CartBean.ShopCarsBean item1 : cartList) {
                    if (item1.isSelect()) {
                        if (shopId1 != item1.getShop().getId()) {
...
//                    cb_select.setImageResource(isShopAllProductSelect ? R.mipmap.icon_checked : R.mipmap.icon_unchecked);
            ...
            iv_remove.setOnClickListener(v -> {
                ...
                int stock12 = item.getProduct().getStock();
                int limit12 = item.getProduct().getLimit();
                int r1 = 0;
                if (limit12 == 0) {
                    r1 = stock12;
                } else {
                    r1 = Math.min(stock12, limit12);
                }
                if (item.getCount() < r1) {
                    iv_add.setImageResource(R.mipmap.icon_cart_add);
                }
...
//                        notifyDataSetChanged();
            ...
            iv_add.setOnClickListener(v -> {
                ...
                int stock13 = item.getProduct().getStock();
                int limit13 = item.getProduct().getLimit();
                if (item.getCount() > 1) {
                    iv_remove.setImageResource(R.mipmap.icon_cart_remove);
                }
                if (limit13 == 0) {
                    if (item.getCount() >= stock13) {
                        ToastUtil.showToast("库存不足", false);
                        iv_add.setImageResource(R.drawable.iv_plus_grey);
                        return;
                    }
                } else if (stock13 > limit13) {
                    if (item.getCount() >= limit13) {
                        ToastUtil.showToast("不能超出限购", false);
                        iv_add.setImageResource(R.drawable.iv_plus_grey);
                        return;
                    }
...
//                    notifyDataSetChanged();
    ...
    public void onrefreshandloadshop() {
        getCarList();
    }
