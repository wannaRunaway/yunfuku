...
public class PaySettingPresenter extends BasePresenter<PaySettingView, PaySettingModel> {
    ...
    PaySettingPresenter(PaySettingView view) {
        setVM(view, new PaySettingModel());
    }
     ...
     * @param requestData 参数
     */
    void appBindWeChat(Map<String, String> requestData) {
        mRxManage.add(
                mModel.appBindWeChat(requestData, new RxSubscriber<String>(mContext) {
                    @Override
                    public void onStart() {
                        super.onStart();
                        mView.showLoading("正在加载,请稍后...");
